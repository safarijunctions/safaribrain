...
export function UsersPanel() {
  ...
  const remove = useMutation({
    mutationFn: (membershipId: string) =>
      api.delete(`/admin/users/${membershipId}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["org-members"] }),
  });

  const resetPassword = useMutation({
    mutationFn: (membershipId: string) =>
      api.post<{ email: string; tempPassword: string }>(
        `/admin/users/${membershipId}/reset-password`,
      ),
    onSuccess: (res) =>
      setTempPasswordNotice({ email: res.email, password: res.tempPassword }),
  });
  ...
  return (
    <div className="space-y-6">
      {tempPasswordNotice && (
        <div className="bg-brass-50 border border-brass-200 p-4 text-sm">
          <p className="font-medium">
            One-time password for {tempPasswordNotice.email}:{" "}
            <code className="bg-white px-1.5 py-0.5 rounded border">
              {tempPasswordNotice.password}
            </code>
          </p>
          <p className="text-xs text-earth-500 mt-1">
            Share this with them directly — it won't be shown again. (Once an
            email/SMS integration is configured, invites will be delivered
      ...
      <div className="flex justify-between items-center">
        <p className="text-sm text-earth-500">
          {data?.length ?? 0} people have access to this organization.
        </p>
        <button
          ...
          <div key={m.id} className="px-5 py-4">
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              {PERMISSIONS.map((p) => (
                <label
                  key={p}
                  className="flex items-center gap-1 text-xs text-earth-600"
                >
                  <input
                    type="checkbox"
...
function InviteForm({
  ...
  const invite = useMutation({
    mutationFn: () =>
      api.post<{ user: { email: string }; tempPassword: string | null }>(
        "/admin/users/invite",
        { fullName, email, role, permissions: [] },
      ),
    onSuccess: onInvited,
  });

  return (
    <div className="border border-sand-200 p-4 bg-forest-50/40 grid grid-cols-1 sm:grid-cols-3 gap-3">
      <div>
      ...
      <div className="sm:col-span-3">
        {invite.isError && (
          <p className="text-xs text-status-full mb-2">
