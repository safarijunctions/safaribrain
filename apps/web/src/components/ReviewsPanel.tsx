...
export function ReviewsPanel() {
  ...
  const moderate = useMutation({
    ...
    }) => api.post(`/admin/reviews/${id}/moderate`, { decision }),
    onSuccess: invalidate,
  });

  const reply = useMutation({
    mutationFn: ({ id, text }: { id: string; text: string }) =>
      api.post(`/admin/reviews/${id}/reply`, { text }),
    onSuccess: invalidate,
  });
    ...
    <div className="space-y-4">
      <div className="flex gap-1 border-b border-sand-200">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-3 py-2 text-xs font-medium border-b-2 -mb-px ${tab === t ? "border-forest-600 text-forest-700" : "border-transparent text-earth-500 hover:text-earth-700"}`}
          >
      ...
      {(moderate.isError || reply.isError) && (
        <p className="text-sm text-status-full">
          {((moderate.error ?? reply.error) as Error).message}
        </p>
      )}
      ...
      <div className="space-y-3">
        {data?.map((r) => (
          <div
            key={r.id}
            className="bg-white border border-sand-200 p-4 space-y-2"
          >
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              {tab === "PENDING" && (
                <div className="flex gap-2 text-xs">
                  <button
                    onClick={() =>
                      moderate.mutate({ id: r.id, decision: "PUBLISH" })
                    }
                    className="bg-moss-600 hover:bg-moss-700 text-white rounded px-3 py-1.5 font-medium"
                  >
                    Publish
                  </button>
                  <button
                    onClick={() =>
                      moderate.mutate({ id: r.id, decision: "REJECT" })
                    }
            ...
            {r.title && (
              <p className="text-sm font-medium text-earth-700">{r.title}</p>
            )}
            {r.body && <p className="text-sm text-earth-600">{r.body}</p>}
            ...
            {tab === "PUBLISHED" && (
              <div className="flex gap-2 pt-1">
                ...
                <button
                  onClick={() =>
                    reply.mutate({ id: r.id, text: replyDrafts[r.id] ?? "" })
                  }
