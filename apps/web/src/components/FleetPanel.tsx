...
const COMPLIANCE_COLORS: Record<Vehicle["complianceStatus"], string> = {
  ...
  EXPIRED: "bg-status-full/10 text-status-full",
  NOT_TRACKED: "bg-sand-100 text-earth-500",
};
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3">
        <p className="text-sm text-earth-500">
          {vehicles?.length ?? 0} vehicle{vehicles?.length === 1 ? "" : "s"} in
          the fleet
        </p>
        {canManage && (
          <button
      ...
      <div className="bg-white border border-sand-200 divide-y overflow-hidden">
        {vehicles?.map((v) => (
          <div
            key={v.id}
            className="px-5 py-4 flex items-center justify-between gap-2 flex-wrap"
          >
            <div>
              <p className="font-medium text-sm">
                {v.name}{" "}
                <span className="text-xs text-earth-400">
                  ({v.registrationNumber} · seats {v.capacity} ·{" "}
              ...
              <p className="text-xs text-earth-500 mt-0.5">
                {v.insuranceExpiry && (
                  <>
                    Insurance {new Date(v.insuranceExpiry).toLocaleDateString()}{" "}
                    ·{" "}
                  </>
                )}
                {v.inspectionExpiry && (
            ...
            <div className="flex items-center gap-3 text-xs">
              <span
                className={`font-medium px-2 py-1 rounded-full ${COMPLIANCE_COLORS[v.complianceStatus]}`}
              >
                {COMPLIANCE_LABELS[v.complianceStatus]}
              </span>
              {canManage && (
                <>
                  <button
...
function VehicleForm({
  ...
  const [capacity, setCapacity] = useState(
    vehicle ? String(vehicle.capacity) : "",
  );
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return vehicle
        ? api.patch(`/fleet/vehicles/${vehicle.id}`, body)
        : api.post("/fleet/vehicles", body);
    },
    onSuccess: onSaved,
  });
  ...
  return (
    <div className="border border-sand-200 p-4 bg-forest-50/40 space-y-3">
      ...
      <div className="flex gap-1.5">
        ...
        <button
          onClick={onCancel}
          className="border border-sand-300 px-4 py-2 text-sm"
        >
          Cancel
        </button>
      </div>
