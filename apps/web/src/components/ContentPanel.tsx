...
const UNITS = [
  "PER_PERSON_PER_DAY",
  "PER_PERSON_PER_ENTRY",
  "PER_VEHICLE_PER_ENTRY",
] as const;
...
export function ContentPanel() {
  ...
  const { data: countries } = useQuery({
    ...
    queryFn: () => api.get<string[]>("/content/places/countries"),
  });
  const { data: places, isLoading } = useQuery({
    queryKey: ["places", countryFilter],
    queryFn: () =>
      api.get<Place[]>(
        `/content/places${countryFilter ? `?country=${encodeURIComponent(countryFilter)}` : ""}`,
      ),
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3">
        <div className="flex items-center gap-2">
          <label className="text-xs text-earth-500">Country</label>
          <select
            className="border border-sand-300 rounded px-2 py-1.5 text-sm"
            value={countryFilter}
              ...
              <div>
                <p className="font-medium text-sm">
                  ...
                  <span className="text-xs text-earth-400">
                    ({p.country} · {p.kind.replace(/_/g, "")})
                  </span>
                </p>
                <p className="text-xs text-earth-500 mt-0.5">
                  {p.feeRules.length} fee rule
                  {p.feeRules.length === 1 ? "" : "s"}
                </p>
              </div>
              <div className="flex items-center gap-3 text-xs">
                ...
                {canManage && (
                  <button
                    ...
                    className="text-status-full hover:underline"
                  >
                    Remove
                  </button>
                )}
...
function PlaceForm({ onSaved }: { onSaved: () => void }) {
  ...
  return (
    <div className="border border-sand-200 p-4 bg-forest-50/40 space-y-3">
...
function FeeRulesEditor({
  ...
  const remove = useMutation({
    mutationFn: (ruleId: string) =>
      api.delete(`/content/places/${place.id}/fee-rules/${ruleId}`),
    onSuccess: onChanged,
  });

  return (
    <div className="mt-3 border-t border-sand-100 pt-3 space-y-2">
      {place.feeRules.length === 0 && (
        <p className="text-xs text-earth-400">No fee rules yet.</p>
      )}
      <ul className="text-xs space-y-1">
        {place.feeRules.map((r) => (
          <li key={r.id} className="flex items-center justify-between gap-2">
            <span>
              {r.label} · {r.residency} {r.ageBand && `/ ${r.ageBand}`} · as of{" "}
              {new Date(r.sourceAsOf).toLocaleDateString()}
            </span>
