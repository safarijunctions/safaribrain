...
const STATUS_COLORS: Record<string, string> = {
  ...
  REJECTED: "bg-status-full/10 text-status-full",
};
...
export function AiDraftPanel() {
  ...
  const { data: drafts, isLoading } = useQuery({
    queryKey: ["ai-itinerary-drafts"],
    queryFn: () => api.get<AiItineraryDraft[]>("/ai/itinerary-drafts"),
  });

  const generate = useMutation({
    mutationFn: () =>
      api.post<AiItineraryDraft>("/ai/itinerary-drafts", {
        prompt,
        durationDays,
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-sand-200 p-4 space-y-3">
        <p className="text-sm text-earth-500">
          Describe a trip and an AI will draft a day-by-day outline. Nothing is
          created until you review and approve it below — the AI never publishes
          anything on its own.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
          <textarea
            className="sm:col-span-3 border border-sand-300 px-3 py-2 text-sm"
            rows={2}
          ...
          <div className="bg-white border border-sand-200 divide-y overflow-hidden">
            {decided.map((d) => (
              ...
              >
                ...
                <span
                  className={`text-xs font-medium px-2 py-1 rounded-full ${STATUS_COLORS[d.status]}`}
                >
                  {d.status}
                </span>
              </div>
...
function DraftReviewCard({
  ...
  const approve = useMutation({
    mutationFn: () =>
      api.post(`/ai/itinerary-drafts/${draft.id}/approve`, {
        title,
        summary,
        days,
      }),
    ...
    setDays((ds) =>
      ds.map((d, i) => {
        ...
        return {
          ...d,
          mealsIncluded: has
            ? d.mealsIncluded.filter((m) => m !== meal)
            : [...d.mealsIncluded, meal],
        };
      }),
    );
  }
  ...
  return (
    <div className="border border-brass-200 p-4 bg-brass-50/40 space-y-3">
      ...
      <div className="space-y-2">
        {days.map((d, i) => (
          <div
            key={i}
            className="border border-sand-200 bg-white p-3 space-y-2"
          >
            <div className="flex items-center gap-2 text-xs font-medium text-forest-700">
              Day {d.dayNumber}
            </div>
            <input
            ...
            <div className="flex gap-3 text-xs">
              {MEALS.map((m) => (
                <label key={m} className="flex items-center gap-1">
                  <input
                    type="checkbox"
                    checked={d.mealsIncluded.includes(m)}
                    onChange={() => toggleMeal(i, m)}
                  />
                  {m}
                </label>
              ))}
      ...
      <div className="flex gap-2">
        <button
          ...
          className="text-sm font-medium bg-moss-600 hover:bg-moss-700 text-white px-4 py-2 disabled:opacity-50"
        >
        ...
        <button
          ...
          className="text-sm font-medium border border-sand-300 px-4 py-2 hover:bg-sand-50"
        >
          Reject
        </button>
      </div>
