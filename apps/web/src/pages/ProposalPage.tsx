...
export function ProposalPage() {
  ...
  const [actionDone, setActionDone] = useState<
    "accepted" | "changes-requested" | null
  >(null);
  ...
  if (isLoading || !data)
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-brass-50 to-moss-50">
        <p className="text-sm text-earth-500">Loading your proposal…</p>
      </div>
    );
      ...
      <div className="relative max-w-2xl mx-auto bg-white rounded-2xl shadow-xl shadow-forest-900/10 border border-white overflow-hidden">
        ...
        <div className="p-7 space-y-7">
            ...
            <section>
              ...
              <ol className="space-y-3 text-sm">
                {data.itinerary.days.map((d) => (
                  <li key={d.id} className="border-l-2 border-brass-400 pl-3.5">
          ...
          <section>
            ...
            <table className="w-full text-sm">
              <tbody>
                ...
                {breakdown.discountAmount > 0 && (
                  <tr className="border-t border-sand-100 text-moss-700">
                ...
                <tr className="border-t-2 border-forest-200">
                  ...
                  <td className="py-2.5 text-right font-display font-semibold text-base text-forest-800 tabular-nums">
                    {breakdown.currency}{" "}
                    {breakdown.totalClientPrice.toLocaleString()}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            {breakdown.feeSourcesAsOf.length > 0 && (
              <p className="text-xs text-earth-400 mt-3">
                ...
                {breakdown.feeSourcesAsOf
                  .map((f) => new Date(f.asOfDate).toLocaleDateString())
                  .join(", ")}{" "}
                — official published rates, subject to change by the relevant
                park authority before booking is confirmed.
              </p>
            )}
            {data.isFrozen && (
              <p className="text-xs text-moss-700 mt-2 font-medium flex items-center gap-1.5">
                <span className="inline-block h-1.5 w-1.5 rounded-full bg-moss-600" />
                This price is confirmed and locked in for your booking — it will
                not change.
              </p>
            )}
          </section>
          ...
          {data.status === "SENT" && !actionDone && (
            <section className="border-t border-sand-200 pt-6 space-y-3">
              <button
                ...
                className="w-full bg-gradient-to-r from-moss-600 to-moss-700 hover:from-moss-700 hover:to-moss-800 text-white font-medium rounded-xl py-3.5 shadow-sm shadow-moss-900/20 transition disabled:opacity-50"
              >
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 border border-sand-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-forest-400 focus:border-transparent"
                  value={note}
                ...
                <button
                  ...
                  className="text-sm font-medium border border-sand-300 rounded-lg px-4 py-2 hover:bg-sand-50 transition"
                >
                  Request changes
                </button>
              </div>
              {(accept.isError || requestChanges.isError) && (
                <p className="text-sm text-status-full text-center">
          ...
          {(data.status === "ACCEPTED" || actionDone === "accepted") && (
            <p className="text-center text-moss-700 font-medium">
              🎉 Accepted — your safari expert will be in touch to confirm next
              steps.
            </p>
          )}
          {(data.status === "CHANGES_REQUESTED" ||
            actionDone === "changes-requested") && (
            <p className="text-center text-brass-700 font-medium">
              Thanks — we've flagged your changes to your safari expert.
            </p>
          )}
        </div>
      </div>
