...
export function TradePage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      <h1 className="font-display text-2xl font-semibold text-clay-800 mb-1">Trade Marketplace</h1>
  ...
  return (
    <div className="space-y-3">
      {data?.map((d) => {
        ...
        return (
          <div key={d.id} className="bg-white border border-stone-200 rounded-xl p-4 shadow-sm shadow-clay-900/5">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="text-right text-xs shrink-0">
                ...
                <p className="font-semibold text-acacia-700">
                  {d.currency} {Number(d.netPricePerSeat).toLocaleString()} net
                </p>
                <p className="text-stone-500">{available} of {d.totalSeats} seats free</p>
              </div>
            </div>
            <button onClick={() => setOpenId(openId === d.id ? null : d.id)} className="mt-2 text-xs text-clay-700 hover:underline">
  ...
  if (result) {
    return (
      <div className="mt-3 border-t border-stone-100 pt-3 text-sm">
        <p className="text-acacia-700 font-medium">Booked — {selected.length} seat(s) at {currency} {netPricePerSeat.toLocaleString()} net each.</p>
                ...
                s.status === "BOOKED"
                  ...
                  : s.status === "HELD" && !s.isMine
                    ...
                    : isSelected
                      ...
                      : "bg-white border-stone-300 hover:border-clay-400"
              }`}
            >
              {s.label}
            </button>
      ...
      {!held ? (
        ...
        <div className="grid grid-cols-2 gap-2 text-xs bg-stone-50 rounded-lg p-3">
          ...
          <button
            ...
            onClick={() => book.mutate()}
            className="col-span-2 font-medium bg-acacia-700 hover:bg-acacia-800 text-white rounded px-3 py-1.5 disabled:opacity-50"
          >
