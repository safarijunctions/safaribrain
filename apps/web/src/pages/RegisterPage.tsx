...
const KINDS = [
  { value: "OPERATOR", label: "Tour Operator", hint: "A company running its own safaris and departures." },
  { value: "GUIDE", label: "Tour Guide", hint: "A freelance guide selling your own itineraries directly to clients." },
  { value: "AGENT", label: "Tour Agent / Agency", hint: "Resell other operators' and guides' inventory at wholesale rates." },
] as const;
      ...
      <form onSubmit={onSubmit} className="relative bg-white/90 backdrop-blur-sm shadow-xl shadow-clay-900/10 rounded-2xl p-8 w-full max-w-md border border-white/60">
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="col-span-2">
            ...
            <input className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={fullName} onChange={(e) => setFullName(e.target.value)} required />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-stone-700 mb-1">
              {kind === "OPERATOR" ? "Company name" : kind === "GUIDE" ? "Your business/guide name" : "Agency name"}
            </label>
            <input className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={organizationName} onChange={(e) => setOrganizationName(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Country</label>
            <input className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={country} onChange={(e) => setCountry(e.target.value.toUpperCase())} placeholder="TZ" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Currency</label>
            <input className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase())} placeholder="USD" required />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-stone-700 mb-1">Email</label>
            <input type="email" className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-stone-700 mb-1">Password</label>
            <input type="password" minLength={8} className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm" value={password} onChange={(e) => setPassword(e.target.value)} required />
          </div>
        </div>
