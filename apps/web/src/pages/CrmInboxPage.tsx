...
const STAGE_COLORS: Record<string, string> = {
  NEW: "bg-savannah-100 text-savannah-700",
  QUOTED: "bg-brass-100 text-brass-700",
  NEGOTIATING: "bg-status-almost-full/10 text-status-almost-full",
  BOOKED: "bg-status-available/10 text-status-available",
  COMPLETED: "bg-sand-200 text-earth-700",
  LOST: "bg-status-full/10 text-status-full",
};
...
export function CrmInboxPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-earth-500">
            One deduplicated contact and request per enquiry, from any channel.
          </p>
        </div>
        <button
          onClick={() => setShowForm((s) => !s)}
          className="self-start sm:self-auto bg-gradient-to-r from-forest-600 to-forest-700 hover:from-forest-700 hover:to-forest-800 text-white text-sm font-medium  px-4 py-2 shadow-sm shadow-forest-900/10 transition"
        >
      ...
      <div className="bg-white border border-sand-200  divide-y shadow-sm shadow-forest-900/5 overflow-hidden">
        {data?.map((r) => (
          <Link
            key={r.id}
            to="/crm/$requestId"
            params={{ requestId: r.id }}
            className="flex items-center justify-between px-5 py-4 hover:bg-forest-50/50 transition"
          >
            <div>
              <p className="font-medium">{r.contact.fullName}</p>
              <p className="text-xs text-earth-500">
                {r.source} · {r.partySize} pax ·{" "}
                {new Date(r.createdAt).toLocaleDateString()}
              </p>
            </div>
            <span
              className={`text-xs font-medium px-2 py-1 rounded-full ${STAGE_COLORS[r.stage] ?? "bg-sand-100"}`}
            >
              {r.stage}
            </span>
          </Link>
...
function NewEnquiryForm({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <form
      onSubmit={submit}
      className="bg-white border border-sand-200  p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4 shadow-sm shadow-forest-900/5"
    >
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setForm({ ...form, contactFullName: e.target.value })
          }
        />
      </div>
      <div>
      ...
      <div>
        ...
        >
          {["WEB", "WHATSAPP", "EMAIL", "PHONE", "REFERRAL", "WALK_IN"].map(
            (s) => (
      ...
      {create.isError && (
        <p className="sm:col-span-2 text-sm text-status-full">
